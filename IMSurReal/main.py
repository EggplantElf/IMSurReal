...
class Realization(object):
    def __init__(self, args):
        ...
        if args.mode == 'train':
            ...
            self.train_sents = read_conllu(self.args.train_file, self.args.ud_train, skip_lost, self.args.orig_word, self.args.first_train)
            ...
            self.dev_sents = read_conllu(self.args.dev_file, self.args.ud_dev, skip_lost, self.args.orig_word)
            self.test_sents = read_conllu(self.args.input_file, self.args.ud_test, skip_lost, self.args.orig_word) if self.args.input_file else []
            if self.args.lemmatize:
            # if 'ko' in self.args.train_file:
                self.log('lemmatize')
                convert_lemma_morph(self.train_sents + self.extra_sents + self.dev_sents + self.test_sents)
    ...
    def train(self):
        ...
        waited = update_time = 0
        best_score = -1
        for step, sent in tqdm(enumerate(iterate_sents(self.train_sents, self.extra_sents, self.args.extra_ratio), 1)):
            self.encode_sent(sent, True)
            loss = 0
            for task in self.args.tasks:
                res = self.decoders[task].train_one_step(sent)
                all_time[task] += res['time']
            ...
            if step % self.args.eval_every == 0:
                ...
                # print(res)
                # self.log(f"[step={step}]\tl2_norm_loss={l2norm_value}\tupdate_time={update_time:.1f}s")
                self.log(f"[step={step}]\tupdate_time={update_time:.1f}s")
                update_time = 0
                for task in self.args.tasks:
                    self.log(f"[step={step}]\t{task}_loss={all_loss[task]/self.args.eval_every:.2f}, "\
                            ...
                            f"train_{task}_time={all_time[task]:.1f}s")
                    self.log(f"dev_{task}_score={100*res[f'{task}_score']:.2f}, time={res[f'{task}_time']:.1f}s")
                    all_time[task] = all_loss[task] = all_correct[task] = all_total[task] = 0
    ...
    def finetune(self, best_scores):        
        ...
        for task in self.args.tasks:
            ...
            ttime = loss = total = correct = waited = 0
            ...
            for step, sent in tqdm(enumerate(iterate_sents(self.train_sents, self.extra_sents, self.args.extra_ratio), 1)):
                ...
                if step % self.args.eval_every == 0:
                    ...
                    self.log(f"[step={step}]\t{task}_loss={loss/self.args.eval_every:.2f}, "\
                            f"train_{task}_time={ttime:.1f}s")
                    self.log(f"dev_{task}_score={100*res['score']:.2f}, time={res[f'time']:.1f}s")
                    ttime = loss = total = correct = 0
...
if __name__ == '__main__':
    ...
    # use the model to predict and write to a output file
        ...
        test_sents = read_conllu(args.input_file, args.ud_test, skip_lost, model.args.orig_word) # lemma, no_word

        # deal with korean lemma
        if model.args.lemmatize:
        # if 'ko' in model.args.train_file:
            print('lemmatize')
